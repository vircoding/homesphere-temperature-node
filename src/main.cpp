...
void onReceivedCallback(const uint8_t* mac, const uint8_t* data, int length) {
  // Procesar mensajes recibidos durante la sincronizacion
  if (length == sizeof(NowManager::SyncBroadcastMsg)) {
    const NowManager::SyncBroadcastMsg* msg =
        reinterpret_cast<const NowManager::SyncBroadcastMsg*>(data);

    if (verifyCRC8(*msg)) {
      config.saveMasterMacConfig(mac);

      Serial.print("MAC del master recibida: ");
      const String macStr = macToString(mac);
      Serial.println(macStr.c_str());

      // Registrar nuevo Master Peer
      now.registerMasterPeer(mac);

      // Enviar registro y reiniciar
      if (now.sendRegistrationData()) {
        ESP.restart();
      }
    } else {
